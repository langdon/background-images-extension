...
const BackgroundImagesPrefsWidget = new Lang.Class({
    Name: 'BackgroundImagesPrefsWidget',
    Extends: Gtk.Grid,
    ...
    _init: function() {
        ...
        this._settings.connect('changed', Lang.bind(this,
            function(settings, key) {
                if (key == 'calendar-image-file' ||
                    key == 'calendar-image-size')
        ...
        let fileChooser = new Gtk.FileChooserButton({ title: "Select an Image",
                                                      filter: filter });
        fileChooser.set_filename(this._settings.get_string('calendar-image-file'));
        fileChooser.connect('file-set', Lang.bind(this,
            function() {
                this._settings.set_string('calendar-image-file',
                                          fileChooser.get_filename());
            }));
        this._addRow(1, "calendar-image image", fileChooser);
        ...
        this._settings.bind('calendar-image-position', comboBox, 'active-id',
                            Gio.SettingsBindFlags.DEFAULT);
        this._addRow(2, "Position", comboBox);

        let adjustment = this._createAdjustment('calendar-image-size', 0.75);
        let scale = new Gtk.Scale({ adjustment: adjustment,
                                    draw_value: false });
        this._addRow(3, "Size", scale);

        adjustment = this._createAdjustment('calendar-image-border', 1.0);
        scale = new Gtk.Scale({ adjustment: adjustment, draw_value: false });
        this._addRow(4, "Border", scale);

        adjustment = this._createAdjustment('calendar-image-opacity', 1.0);
        scale = new Gtk.Scale({ adjustment: adjustment, draw_value: false });
        this._addRow(5, "Opacity", scale);

        let checkWidget = new Gtk.CheckButton({ label: "Show for all backgrounds" });
        this._settings.bind('calendar-image-always-visible', checkWidget, 'active',
                            Gio.SettingsBindFlags.DEFAULT);
    ...
    _drawPreview: function(preview, cr) {
        ...
        if (!this._calendarImage)
            this._createLogoThumbnail(width, height);

        let [x, y] = this._getLogoPosition(width, height);
        Gdk.cairo_set_source_pixbuf(cr, this._calendarImage, x, y);
        cr.paintWithAlpha(this._settings.get_uint('calendar-image-opacity') / 255.0);
    },
    ...
    _createLogoThumbnail: function(width, height) {
        ...
        let pixbuf = GdkPixbuf.Pixbuf.new_from_file(file.get_path());
        let size = this._settings.get_double('calendar-image-size') / 100;
        let ratio = pixbuf.get_width() / pixbuf.get_height();
        this._calendarImage = pixbuf.scale_simple(size * width,
                                         size * width / ratio,
                                         GdkPixbuf.InterpType.BILINEAR);
    },

    _getLogoPosition: function(width, height) {
        let scaledBorder = this._settings.get_uint('calendar-image-border') * this._scale;
        let x, y;
        switch (this._settings.get_string('calendar-image-position')) {
            case 'center':
                x = (width - this._calendarImage.get_width()) / 2;
                y = (height - this._calendarImage.get_height()) / 2;
                break;
            case 'bottom-left':
                x = scaledBorder;
                y = height - this._calendarImage.get_height() - scaledBorder;
                break;
            case 'bottom-center':
                x = (width - this._calendarImage.get_width()) / 2;
                y = height - this._calendarImage.get_height() - scaledBorder;
                break;
            case 'bottom-right':
                x = width - this._calendarImage.get_width() - scaledBorder;
                y = height - this._calendarImage.get_height() - scaledBorder;
                break;
        }
        return [x, y];
...
function buildPrefsWidget() {
    let widget = new BackgroundImagesPrefsWidget();
    widget.show_all();
